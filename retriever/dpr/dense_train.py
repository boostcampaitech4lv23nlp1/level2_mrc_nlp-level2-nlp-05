...
class DenseRetrievalTrainer:
    def __init__(
        ...
        self.wiki_tokens = self.tokenizer(
            self.wiki_corpus, padding="max_length", truncation=True, return_tensors="pt"
        )
        ...
        if self.cfg.training_args.in_batch_neg:
            ...
            for train_step, batch in enumerate(epoch_iterator):
                ...
                p_inputs = {
                    "input_ids": batch[0].view(
                        ...
                        * (self.cfg.training_args.num_neg + 1),
                        -1,
                    ),
                ...
                epoch_iterator.set_description(
                    "Loss %.04f step %d" % (loss, train_step)
                )
            ...
            for train_step, batch in enumerate(epoch_iterator):
                ...
                p_inputs = {
                    "input_ids": batch[0].view(
                        self.args.per_device_train_batch_size
                        * (self.cfg.training_args.num_neg + 2),
                        -1,
                    ),
                    "attention_mask": batch[1].view(
                        self.args.per_device_train_batch_size
    ...
    def train(
        ...
        scheduler = get_linear_schedule_with_warmup(
            optimizer,
            num_warmup_steps=t_total * self.args.warmup_ratio,
            ...
            if (epoch + 1) % 1 == 0:
                ...
                if top_100 > best_score:
                    self.p_encoder.save_pretrained(
                        "./retriever/saved_models/p_encoder_{args.cfg.training_args.p_encoder_save_name}"
                    )
                    self.q_encoder.save_pretrained(
                        "./retriever/saved_models/q_encoder_{args.cfg.training_args.q_encoder_save_name}"
                    )
    ...
    def build_faiss(self, p_encoder_path=None, num_clusters=64):
        ...
        p_encoder = (
            self.p_encoder
            if p_encoder_path == None
            else BertEncoder.from_pretrained(p_encoder_path)
        )
...
def main(cfg):
    ...
    DR_trainer.train()
    DR_trainer.build_faiss(
        p_encoder_path="./retriever/saved_models/p_encoder", num_clusters=32
    )
