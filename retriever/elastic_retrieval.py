...
class ElasticRetrieval:
    def __init__(
        ...
        self.context_path = (
            "wikipedia_documents.json" if context_path is None else context_path
        )
        self.setting_path = (
            "./retriever/setting.json" if setting_path is None else setting_path
        )
        self.index_name = index_name

        # wiki_document.json 전용 elastic search document 삽입 코드
        if index_name:
            if self.es.indices.exists(index=index_name) and (
                self.es.count(index=index_name)["count"] > 1000
            ):
                # index_name이 현재 존재하고 제대로 데이터가 삽입이 된 상태일때(1000개 이상정도면 이상 없다고 판단)
                self.index_name = index_name
                print(f" ### {index_name} 에서 search를 시작합니다 ###")
                print(
                    f" {index_name} 총 데이터 개수 : {self.es.count(index=self.index_name)['count']}"
                )
