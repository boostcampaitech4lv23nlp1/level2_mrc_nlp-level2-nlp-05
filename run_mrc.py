...
def run_mrc(
    ...
    def post_processing_function(
        ...
        # prediction 결과에서 뒷 부분의 조사를 제거합니다.
        mecab = Mecab()
        okt = Okt()
        hannanum = Hannanum()
    ...
    if training_args.do_predict:
        ...
        predict_dataset = load_eval_dataset(predict_datasets, max_seq_length, data_args, tokenizer)
        logger.info("*** Predict ***")
        predictions = trainer.predict(test_dataset=predict_dataset, test_examples=predict_datasets["validation"])
        # predictions.json 은 postprocess_qa_predictions() 호출시 이미 저장됩니다.
        print("No metric can be presented because there is no correct answer given. Job done!")
