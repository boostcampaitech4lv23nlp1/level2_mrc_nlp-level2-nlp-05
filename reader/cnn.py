...
class Conv1DRobertaForQuestionAnswering(RobertaPreTrainedModel):
    ...
    def __init__(self, pretrained_model_name_or_path, config, model_args):
        ...
        self.qa_outputs_cnn = nn.Linear(in_features=384 * 3, out_features=config.num_labels)
...
class Conv1DElectraForQuestionAnswering(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        '''
        electraForQuestionAnswering에서 수정 시작
        원본 코드 : 
        ...
        # discriminator_hidden_states[0] = [batch_size,max_sequence_length=384, hidden_dim=1024]
        sequence_output = discriminator_hidden_states[0].permute(0, 2, 1)   #차원바꾸기 [batch_size,hidden_dim,max_sequence_length]
        ...
        '''
        electraForQuestionAnswering에서 수정 끝
        '''
        start_logits, end_logits = logits.split(1, dim=-1)
        start_logits = start_logits.squeeze(-1).contiguous()
        end_logits = end_logits.squeeze(-1).contiguous()
